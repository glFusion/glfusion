...
// +---------------------------------------------------------------------------+
// | $Id:: gd_image.php 1312 2007-10-20 23:22:42Z mevans0263                  $|
// |                                                                           |
// | GD Lib v2 Graphic Library interface for Media Gallery                     |
// +---------------------------------------------------------------------------+
...
if (strpos ($_SERVER['PHP_SELF'], 'gd_image.php') !== false)
{
...
function _mg_resizeImage($srcImage, $destImage, $sImageHeight, $sImageWidth, $dImageHeight, $dImageWidth, $mimeType) {
    ...
    if ( $_MG_CONF['verbose'] ) {
        COM_errorLog("MG_resizeImage: Resizing using GD libs src = " . $srcImage . " mimetype = " . $mimeType);
    }
    switch ( $mimeType ) {
        case 'image/jpeg' :
        case 'image/jpg' :
        ...
        case 'image/x-targa' :
        case 'image/tga' :
            COM_errorLog("MG_resizeImage: TGA files not supported by GD Libs");
            return array(false,'TGA format not supported by GD Libs');;
        default :
            COM_errorLog("MG_resizeImage: GD Libs only support JPG, PNG, and GIF image types.");
            return array(false,'GD Libs only support JPG, PNG and GIF image types');
    }
    if ( !$image ) {
        COM_errorLog("MG_resizeImage: GD Libs failed to create working image.");
...
function _mg_RotateImage($srcImage, $direction,$mimeType) {
    ...
    $rc = _mg_gdRotate($srcImage,$tmpImage,$GD_rotate,$mimeType);
    if ( $rc == true ) {
...
function _mg_gdRotate( $src, $dest, $angle, $mimeType ) {
    ...
    switch ( $mimeType ) {
        ...
        case 'image/x-targa' :
        case 'image/tga' :
            COM_errorLog("MG_gdRotate: TGA files not supported by GD Libs");
            return array(false,'TGA files not supported by GD Libs');;
        default :
            COM_errorLog("MG_gdRotate: GD Libs only support JPG, PNG, BMP, and GIF image formats.");
...
function _mg_convertImageFormat($srcImage,$destImage,$destFormat,$mimeType) {
    ...
    switch ( $mimeType ) {
        ...
        case 'application/psd' :
            COM_errorLog("MG_convertImageFormat: GD Libs only support JPG, PNG, BMP, and GIF source formats.");
            return array(false,'GD Libs only support JPG, PNG, BMP, and GIF source formats.');
    }
    if ( !$image ) {
        COM_errorLog("MG_convertImageFormat: GD Libs returned an error reading source image: " . $srcImage);
        return array(false,'GD Libs returned an error reading source image.');
    }

    $imgsize = @getimagesize($srcImage);
    if ($imgsize == false ) {
        COM_errorLog("MG_convertImageFormat: GD Libs unable to determine dimensions for source image: " . $srcImage);
    ...
    if ($_MG_CONF['verbose'] ) {
        COM_errorLog("MG_convertImageFormat: Outputting new image " . $destImage . " Format: " . $destFormat);
    }
    ...
    if ( $rc == true ) {
        ...
        COM_errorLog("MG_convertImageFormat: GD Libs returned an error during conversion.");
...
function _mg_watermarkImage($origImage, $watermarkImage, $opacity, $location, $mimeType ) {
    ...
    if ( $_MG_CONF['verbose'] ) {
        COM_errorLog("MG_watermarkImage: Using GD Libs to watermark image.");
    }
    ...
    switch( $file_extension ) {
        ...
        case "jpg":
            ...
            break;
        default :
            COM_errorLog("MG_watermarkImage: Unable to apply watermark, unrecognized filetype for watermark image");
    ...
    if ( $size != false ) {
        ...
        switch ( $mimeType ) {
            ...
            case 'image/x-targa' :
            case 'image/tga' :
                COM_errorLog("MG_watermarkImage: TGA files not supported by GD Libs");
                return array(false,'TGA files not supported by GD Libs');
            default :
                COM_errorLog("MG_watermarkImage: GD Libs only support JPG, PNG, and GIF image types.");
                return array(false,'GD Libs only support JPG,PNG, and GIF image types');
        }
        if ( $image == false ) {
        ...
        if ( $_MG_CONF['jhead_enabled'] == 1 ) {
            $rc = MG_execWrapper('"' . $_MG_CONF['jhead_path'] . "/jhead" . '"' . " -te " . $origImage . " " . $newSrcTmp);
        }
        @unlink($origImage);
        ...
        COM_errorLog("MG_watermarkImage: Watermark successfully applied (GD libs)");
    } else {
        COM_errorLog("MG_watermarkImage: Unable to determine src image filesize for watermarking (GD libs)");
