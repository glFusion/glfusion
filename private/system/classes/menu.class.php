...
class menu {
    ...
    function editTree( $depth, $count ) {
        ...
        foreach ( $this->menu_elements as $element ) {
            $elementArray = $element->_editElement($depth,$count);
            if ( $elementArray != NULL ) {
...
//@FIXME - Remove??
    ...
    function _editElement( $depth, $count ) {
        global $_CONF, $level;
        ...
        if ( !empty($this->children)) {
            ...
            if ( is_array($children) ) {
                foreach($children as $child) {
                    $level++;
                    if ( is_object($child) ) {
                        $carray = $child->_editElement($depth,$count);
    ...
    function _parseElement(  )
    {
        ...
        switch ( $this->type ) {
            ...
            case ET_FUSION_MENU :
                switch ($this->subtype) {
                    case USER_MENU :
                        $item_array = $this->_userMenu();
                        break;

                    case ADMIN_MENU :
                        $item_array = $this->_adminMenu();
                        break;

                    case TOPIC_MENU :
                        $item_array = $this->_topicMenu();
                        break;
