...
function MG_haveEXIF( $mid ) {
    ...
    $result = DB_query("SELECT media_filename,media_mime_ext,media_exif FROM {$_TABLES['mg_media']} WHERE media_id='" . $mid . "'");
    ...
    if ( count($exif) == 0 ) {
        DB_query("UPDATE {$_TABLES['mg_media']} SET media_exif=0 WHERE media_id='" . $mid . "'");
        return 0;
    }
...
function MG_readEXIF( $mid, $columns = 2, $mqueue=0) {
    ...
    $media_filename = DB_getItem($mqueue ? $_TABLES['mg_mediaqueue'] : $_TABLES['mg_media'],'media_filename',"media_id='" .$mid."'");
    if ( $media_filename == '' ) {
        return '';
    }
    $media_mime_ext = DB_getItem($mqueue ? $_TABLES['mg_mediaqueue'] : $_TABLES['mg_media'],'media_mime_ext',"media_id='" . $mid . "'");
