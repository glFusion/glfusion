...
if (strpos ($_SERVER['PHP_SELF'], 'exif.php') !== false)
{
...
function formatData($type,$tag,$intel,$data) {
	...
	if($type=="ASCII") {
		...
		if(($tag=="011a" || $tag=="011b") && $bottom==1) { //XResolution YResolution
			$data=$top.$LANG_MG04['dots_per_resolution_unit'];
		} else if($tag=="829a") { //Exposure Time
	        if ($top/$bottom < 1)
			    $data="1/".round($bottom/$top, 0).$LANG_MG04['sec'];
      	    else
      	 	    $data = sprintf("%d %s", $top / $bottom, $LANG_MG04['sec']);
//			if($top/10 == 1) $data="1/".round($bottom/10, 0).$LANG_MG04['sec'];
//			else $data=$top."/".$bottom.$LANG_MG04['sec'];
		} else if($tag=="829d") { //FNumber
			$data="f ".$data;
		} else if($tag=="9204") { //ExposureBiasValue
			$data=$data.$LANG_MG04['ev'];
		...
		if($tag=="0112") { //Orientation
			if($data==1) $data = $LANG_MG04['normal'];
			if($data==2) $data = $LANG_MG04['mirrored'];
			if($data==3) $data = $LANG_MG04['upsidedown'];
			if($data==4) $data = $LANG_MG04['upsidedown_mirrored'];
			if($data==5) $data = $LANG_MG04['cw_mirrored'];
			if($data==6) $data = $LANG_MG04['ccw'];
			if($data==7) $data = $LANG_MG04['ccw_mirrored'];
			if($data==8) $data = $LANG_MG04['cw'];
			...
			if($data==2) $data = $LANG_MG04['inch'];
			if($data==3) $data = $LANG_MG04['centimeter'];
			...
			else if($data==10) $data = $LANG_MG04['flash'];
...
function read_entry(&$result,$in,$seek,$intel,$ifd_name,$globalOffset) {
	...
	if($tag_name=="MakerNote") { //if its a maker tag, we need to parse this specially
		...
		if(eregi("NIKON",$make)) {
			require_once $_CONF['path'] . 'plugins/mediagallery/include/makers/nikon.php';
			parseNikon($data,$result);
			$result[$ifd_name]['KnownMaker'] = 1;
		} else if(eregi("OLYMPUS",$make)) {
			require_once $_CONF['path'] . 'plugins/mediagallery/include/makers/olympus.php';
			parseOlympus($data,$result,$seek,$globalOffset);
			$result[$ifd_name]['KnownMaker'] = 1;
		} else if(eregi("Canon",$make)) {
			require_once $_CONF['path'] . 'plugins/mediagallery/include/makers/canon.php';
			parseCanon($data,$result,$seek,$globalOffset);
			$result[$ifd_name]['KnownMaker'] = 1;
		} else if(eregi("FUJIFILM",$make)) {
			require_once $_CONF['path'] . 'plugins/mediagallery/include/makers/fujifilm.php';
			parseFujifilm($data,$result);
			$result[$ifd_name]['KnownMaker'] = 1;
		} else if(eregi("SANYO",$make)) {
			require_once $_CONF['path'] . 'plugins/mediagallery/include/makers/sanyo.php';
		...
		require_once $_CONF['path'] . 'plugins/mediagallery/include/makers/gps.php';
		...
		$formated_data = formatData($type,$tag,$intel,$data);
// print "tag = " . $tag . " data = " . $formated_data . '<br />';
