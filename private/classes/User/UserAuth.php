...
final class UserAuth extends User {
     ...
     *
     * User has submitted their username / password - attempt to login or redirect
     * to the login page with appropriate messages.
     *
     * Throttling is handled in the internal methods that perform the actual login
     *
     * Throws:
     *  - LocalLoginServiceDisabledException - local login disabled
     *  -
     *
    ...
    public function userLocalLoginController()
    {
        ...
        if (empty($loginname) || empty($passwd)) {
            // check if we have enough capacity to try and log in again...
            // this will throw TooManyRequestsException if we do not have the capacity..
     ...
     */
    private function userFinalLogin($local_login = false, $redirect = true)
    {
...
//@TODO - move to userCreate?
    ...
    public function confirmEmail($selector, $token) {
        ...
        if (!empty($confirmationData) && $confirmationData != null) {
...
//					Log::write('system',Log::DEBUG,'User is not verified or status is not 3 :: ' . $userData['status']);
                    ...
                    } elseif ($userData['status'] === Status::PENDING_REVIEW) {
                        throw new Exceptions\AccountPendingReviewException();
                    } else {
                        throw new Exceptions\EmailNotVerifiedException();
                    }
...
//			return null;
    ...
    /**
     * Merge a remote account with an existing local account
     */
    public function userMergeAccounts()
    {
        ...
        if (SEC_check_hash($localpwd, $userData['passwd'])) {
            ...
            \glFusion\Cache\Cache::getInstance()->delete($cacheKey);
            ...
            $this->userFinalLogin(true, false);
