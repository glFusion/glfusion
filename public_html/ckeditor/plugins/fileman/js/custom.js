...
function FileSelected(file){
  ...
  $(window.parent.document).find('#' + fieldId+'_img').attr('src', file.fullPath);
  $(window.parent.document).find('#' + fieldId).attr('value', file.fullPath);
//  $(window.parent.document).find('#' + fieldId).attr('value', file.fullPath);
//  $(window.parent.document).find('#' + fieldId).value = file.fullPath;
//  $('input.'+fieldId).val(File.fullPath);
  window.parent.closeStoryImage();
}
function FileSelectedxx(file){
  /**
   * file is an object containing following properties:
   *
   * fullPath - path to the file - absolute from your site root
