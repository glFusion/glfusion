...
class Filemanager {
    ...
    public function __construct($extraConfig = '') {
        ...
        $this->__log(__METHOD__ . '#1 $this->root value ' . $this->root);
        $this->__log(__METHOD__ . '#2 $this->path_to_files ' . $this->path_to_files);
        $this->__log(__METHOD__ . '#3 $this->doc_root value ' . $this->doc_root);
        $this->__log(__METHOD__ . '#4 $this->separator value ' . $this->separator);
...
private function is_valid_path($path) {
  ...
  // @see http://stackoverflow.com/questions/5642785/php-a-good-way-to-universalize-paths-across-oss-slash-directions
    ...
    $path_to_files = trim(realpath($this->path_to_files));
    if ( $path_to_files[strlen($path_to_files)-1] != '/' ) {
        $path_to_files = $path_to_files.'/';
    }
    $real_path = trim(realpath($path));
    if ( $real_path[strlen($real_path)-1] != '/' ) {
        $real_path = $real_path.'/';
    }
    return substr(realpath($real_path), 0, strlen($path_to_files)-1) == realpath($path_to_files);
}
