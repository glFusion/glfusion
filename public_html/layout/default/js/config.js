...
console.log('tabsWidth = %d',tabsWidth);

        $("#tabs > ul > li").each(function(idx) {
            totalWidth += ($(this).width() + 5);
console.log('totalWidth = %d', totalWidth);
            if (totalWidth >= tabsWidth && overflowAt === null) {
console.log('overflow found at idx %d',idx);
                overflowAt = idx;
            }
        });
...
//console.log('idxAfter is equal to %d',idxAfter);
        dropDown = '';
        if ( idxAfter > 0 ) {
            idxAfter -= 1;
...
//console.log('currenTab.length = ' + currenTab.length);
//console.log('    tab title = %s', currenTab.text());
                if ( currenTab.length ) {
...
//console.log('tab href = %s',currenTabHref);
// hiddentabs created here
                        hiddenTabs[currenTabHref] = {
                            'tab_title': currenTab.text(),
                            'tab_content': currenTabContent.html()
                        };
//console.log('Tab index is %d',i);
...
//console.log('building li for dropdown for %s',tab);
// we are stacking them, with the first processed on the bottom
// see that we + dropDown after adding, so this puts the later
// ones in hiddentabs on infront of each other
