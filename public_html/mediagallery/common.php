...
function MG_getValidFileTypes( $album_id ) {
    ...
    if( $valid_formats & MG_OTHER ) {
        ...
        $valid_types = '';
        $valid_types .= ( $valid_formats & MG_JPG ) ? '*.jpg;*.jpeg;' : '';
        $valid_types .= ( $valid_formats & MG_PNG ) ? '*.png;' : '';
        $valid_types .= ( $valid_formats & MG_TIF ) ? '*.tif;*.tiff;' : '';
        $valid_types .= ( $valid_formats & MG_GIF ) ? '*.gif;' : '';
        $valid_types .= ( $valid_formats & MG_BMP ) ? '*.bmp;*.dib;' : '';
        $valid_types .= ( $valid_formats & MG_TGA ) ? '*.tga;*.tpic;' : '';
        $valid_types .= ( $valid_formats & MG_PSD ) ? '*.psd;*.pdd;' : '';
        $valid_types .= ( $valid_formats & MG_MP3 ) ? '*.mp3;' : '';
        $valid_types .= ( $valid_formats & MG_OGG ) ? '*.ogg;*.ogv;*.oga;*.ogx;*.spx;' : '';
        ...
        $valid_types .= ( $valid_formats & MG_MPG ) ? '*.mpg;*.mpeg;*.mp1;*.mp2;*.m1v;*.mp1;*.m2a;*.mpa;*.mpv;' : '';
        $valid_types .= ( $valid_formats & MG_FLV ) ? '*.flv;*.f4v;*.f4p;*.f4a;*.f4b;' : '';
        $valid_types .= ( $valid_formats & MG_ZIP ) ? '*.zip;*.zipx;' : '';
