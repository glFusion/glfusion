...
if ( isset($_POST['submit']) ) {
    ...
    if ( $sender != 0 ) {
        ...
        $result=DB_query("SELECT COUNT(*) FROM {$_FM_TABLES['filemgmt_brokenlinks']} WHERE lid='".DB_escapeString($lid)."' AND sender='".intval($sender)."'");
        ...
        $result=DB_query("SELECT COUNT(*) FROM {$_FM_TABLES['filemgmt_brokenlinks']} WHERE lid='".DB_escapeString($lid)."' AND ip='".DB_escapeString($ip)."'");
        list ($count)=DB_fetchARRAY($result);
        if ( $count > 0 ) {
            redirect_header("index.php",2,_MD_ALREADYREPORTED);
            exit();
    ...
    DB_query("INSERT INTO {$_FM_TABLES['filemgmt_brokenlinks']} (lid, sender, ip) VALUES ('".DB_escapeString($lid)."', '".DB_escapeString($sender)."', '".DB_escapeString($ip)."')") or die('');
