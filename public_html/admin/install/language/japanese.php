...
$LANG_INSTALL = array(
    0 => 'glFusion - 究極のWeblogシステム',
    1 => 'インストールのサポート',
    2 => '究極のWeblogシステム',
    3 => 'glFusionインストール',
    4 => 'PHP 4.1.0以降が必要',
    5 => '申し訳ありません, glFusionは少なくともPHP 4.1.0が動作に必要です。(あなたのご使用中のバージョンは ',
    6 => '). <a href="http://www.php.net/downloads.php">PHPのアップグレード</a>からインストールするか、ご利用のホスティングサービスにご依頼下さい。',
    7 => 'glFusionのファイルの場所を確認できません。',
    8 => 'インストールスクリプトは重要なglFusionのファイルを確認できませんでした。おそらく、デフォルトの場所から変更されていると思います。以下のファイルのあるパスを指定して下さい:',
    9 => 'ようこそ、そしてglFusionを選んでくれてありがとう!',
    10 => 'ファイル/ディレクトリ',
    11 => 'パーミッション',
    12 => '変更',
    13 => '現在',
    14 => 'ディレクトリを変更',
    15 => 'glFusionヘッドラインのエクスポートは無効としました。<code>backend</code>ディレクトリはテストしていません',
    17 => 'ユーザの写真は無効です。<code>userphotos</code>はテストしていません',
    18 => '記事に画像の使用は無効です。<code>articles</code>ディレクトリはテストしていません',
    19 => 'glFusionでは、Webサーバがいくつかのファイルとディレクトリに書き込みできる必要があります。以下は、変更が必要なファイルとディレクトリです。',
    20 => '警告!',
    21 => 'あなたのglFusionとサイトは上のエラーを直すまで適切に動作しません。この段階に従わないのが、glFusionを使おうとしたとき起きるエラーの「ナンバー1」の理由です。どうか、続ける前に必要な変更を行なって下さい。',
    22 => '不明',
    23 => 'インストール方法を選択:',
    24 => '新規インストール',
    25 => 'アップグレード',
    ...
    52 => '申し訳ありませんが、glFusionを動作させるには最低でもMySQL 3.23.2が必要です(現在のバージョンは ',
    ...
    61 => '戻る',
    62 => '続ける',
    63 => 'glFusionのデータベースがすでに存在します。インストーラは既存のglFusionのデータベースに、新規のインストールを行うことは許可されていません。続けるには次のどれかを行わなければなりません:',
    64 => '存在するデータベースからテーブルを削除します。または、データベースを削除してから再作成します。それから下の「再実行」ボタンをクリックします。',
    65 => '下の「アップグレード」オプションを選択して、データベースを(新しいglFusionのバージョンに)アップグレードを行います。',
    66 => '再実行',
    67 => 'glFusionのデータベースの設定でエラー',
    68 => 'データベースが空hdeありません。すべてのテーブルを削除して、もう一度初めてください。',
    69 => 'glFusionをアップグレード中',
    70 => '始める前に、データベースと現在のglFusionのファイルのバックアップがたいへん重要です。このインストール処理ではglFusionのデータベースを作り変えるため、何かうまくいかなくてアップグレードの処理をやり直すときに、元のデータベースのバックアップが必要となります。<strong>あなたは警告を受けました!</strong>',
    71 => '以下からアップグレードしようとしている正しいglFusionのバージョンを選択していることを確かめて下さい。このバージョンから進める方向に、ここでの処理を行います。(すなわち、どの古いバージョンからも直接',
    72 => 'にアップグレードできます)。',
    73 => 'glFusionのベータやリリース候補のバージョンからはアップグレードできないことに注意して下さい。',
    74 => 'データベースはすでに更新済みです!',
    75 => 'あなたのデータベースはすでに最新になっています。おそらく、アップグレードを以前に実行されています。もし、アップグレードをやり直す場合は、バックアップしたデータベースを再インストールした上で再度行って下さい。',
    76 => '現在のglFusionのバージョンを選択',
    77 => 'インストーラはglFusionの現在のバージョンを確認できませんでした。どうぞ、次のリストから選択してください。',
    78 => 'アップグレード・エラー',
    79 => 'glFusionのアップグレードのインストール中にエラーが発生しました。',
    80 => '変更',
    81 => '停止!',
    82 => '以下にリストされたファイルのパーミッションを変更することが重要です。そうするまで、glFusionはインストールできません。',
    83 => 'インストール・エラー',
    84 => 'パスの "',
...
$LANG_HELP = array(
    0 => 'glFusionインストールのサポート',
    1 => 'あなたのWebサイト名です。',
    2 => '簡単なWebサイトの説明です。',
    3 => 'glFusionはMySQLかMicrosoft SQLデータベースのいずれかを使用してインストールできます。どちらを選択すべきか確かでない場合はホスティング・サービスの提供者に確認してみて下さい。
<br><br><strong>注意:</strong>InooDBテーブルは(たいへん)巨大なサイトでは性能を改善するかもしれませんが、データベースのバックアップをより複雑にします。',
    ...
    8 => 'ユーザによっては、同じデータベースに複数のglFusionのコピーをインストールしたい場合があります。それぞれのglFusionのコピーが正しく動作するためには、それぞれが一意となるテーブルのプリフィックス(すなわちgl1_やgl2_といった)を指定しなければなりません。',
    9 => 'あなたのサイトの正しいURLであることを確認して下さい。すなわち、glFusionの<code>index.php</code>が位置する(最後にスラッシュをつけない)場所となります。',
    10 => 'ホスティング・サービスによっては、予めadminディレクトリが設定されている場合があります。このような場合は、glFusionのadminディレクトリを何か別の"myadmin"といった名前に変更して、以下のURLも合わせて変更する必要があります。glFusionの管理メニューのアクセスに問題に出会うまでは、そのままにしてください。',
    11 => 'これはglFusionによって送信されるすべてのメールの返信アドレスと、シンジケーション・フィードに表示される連絡先となります。',
