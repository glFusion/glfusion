...
/* --- Updated --- */
    'system_path_prompt'        => 'Enter the full, absolute path on your server to glFusion\'s <strong>private/</strong> directory.<br /><br />For a new install, this is the directory that contains the <strong>db-config.php.dist</strong> file, or for an upgrade, and existing <strong>db-config.php</strong> file.<br /><br />Directory Examples:<br />/home/www/glfusion/private/<br />c:/www/glfusion/private/<br /><br /><strong>Hint:</strong> From a security perspective, the most desirable location for the private/ directory is outside of the webroot.  The webroot is the directory that is served by your web host that relates to the root url of your site (http://www.yoursite.com/).<br /><br />It appears that the absolute path to your <strong>public_html/</strong> <i>(not <strong>private/</strong>)</i> directory is:<br /><br />%s<br /><br />If your web host allows the placement of files <strong>outside</strong> of your webroot, we suggest you place the <strong>private/</strong> directory in this space.<br /><br />If you web host does not allow for placement of files outside of the webroot, please follow the instructions in the <a href="http://www.glfusion.org/wiki/doku.php/glfusion:installation:webroot" target="_blank">documentation</a> on installing the private folder <strong>inside</strong> the webroot.<br /><br /><strong>Advanced Settings</strong> allow you to override some of the default paths.  Generally you do not need to modify or specify these paths, the system will determine them automatically.',
/* --- New --- */
