...
if ( $id_sent == 0 ) {
    $order_select .= '<option value="0">' . $LANG_MB01['top_level'] . '</option>' . LB;
} else {
    $order_select .= '<option value="0">' . $LANG_MB01['first_position'] . '</option>' . LB;
}
$result = DB_query("SELECT id,element_label,element_order FROM {$_TABLES['mb_elements']} WHERE menu_id='" . $menu . "' AND pid=" . $id_sent . ' ORDER BY element_order ASC');
