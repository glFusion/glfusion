...
require_once($_CONF['path'] . '/plugins/menubuilder/functions.inc');

function MB_return_bytes($val) {
    $val = trim($val);
...
if (!SEC_inGroup('Root')) {
    ...
    COM_errorLog("Someone has tried to illegally access the Menu Builder install/uninstall page.  User id: {$_USER['uid']}, Username: {$_USER['username']}, IP: " . $_SERVER['REMOTE_ADDR'],1);
...
*
*/
function plugin_install_menubuilder()
{
...
$T->set_var('install_header', $LANG_MB00['install_header']);
$T->set_var('img',MB_getImageFile('menubuilder.png'));
$T->set_var('cgiurl', $_CONF['site_admin_url'] . '/plugins/menubuilder/install.php');
$T->set_var('admin_url', $_CONF['site_admin_url'] . '/plugins/menubuilder/index.php');

if ($action == 'install') {
    if (plugin_install_menubuilder()) {
        $installMsg = sprintf($LANG_MB00['install_success'],$_CONF['site_admin_url'] . '/plugins/menubuilder/index.php');
        $T->set_var('installmsg1',$installMsg);
    } else {
       	echo COM_refresh ($_CONF['site_admin_url'] . '/plugins.php?msg=72');
    }
} else if ($action == "uninstall") {
   plugin_uninstall_menubuilder('installed');
   $T->set_var('installmsg1',$LANG_MB00['uninstall_msg']);
}

if (DB_count($_TABLES['plugins'], 'pi_name', 'menubuilder') == 0) {

    $errCheck = 0;

    $T->set_var('installmsg2', $LANG_MB00['uninstalled']);
    $T->set_var('readme', $LANG_MB00['readme']);
    $T->set_var('btnmsg', $LANG_MB00['install']);
    ...
    $memory_limit = MB_return_bytes(ini_get('memory_limit'));

    $glversion = explode(".", VERSION);
    if ( $glversion[1] < 4 ) {
        $versionCheck = '<div style="background-color:#ffff00;color:#000000;vertical-align:middle;padding:5px;"><img src="images/redX.png" alt="error" style="padding:5px;vertical-align:middle;">&nbsp;' . $LANG_MB00['gl_version_error'] . '</div>';
        $errCheck++;
    } else {
        $versionCheck = '<div style="vertical-align:middle;padding:5px;"><img src="images/check.png" alt="OK" style="padding:5px;vertical-align:middle;">' . $LANG_MB00['gl_version_ok'] . '</div>';
    }
    if ( $tc_installed == 0 ) {
        $errCheck++;
        $cacheCheck = '<div style="background-color:#ffff00;color:#000000;vertical-align:middle;padding:5px;"><img src="images/redX.png" alt="error" style="padding:5px;vertical-align:middle;">&nbsp;' . $LANG_MB00['tc_error'] . '</div>';
    } else {
        $cacheCheck = '<div style="vertical-align:middle;padding:5px;"><img src="images/check.png" alt="OK" style="padding:5px;vertical-align:middle;">' . $LANG_MB00['tc_ok'] . '</div>';
    }
    if ( $memory_limit < 50331648 ) {
        $memoryCheck = '<div style="background-color:#ffff00;color:#000000;vertical-align:middle;padding:5px;"><img src="images/redX.png" alt="error" style="padding:5px;vertical-align:middle;">&nbsp;' . $LANG_MB00['ml_error'] . '</div>';
    } else {
        $memoryCheck = '<div style="vertical-align:middle;padding:5px;"><img src="images/check.png" alt="OK" style="padding:5px;vertical-align:middle;">' . $LANG_MB00['ml_ok'] . '</div>';
    }

    $glver  = sprintf($LANG_MB00['geeklog_check'],$gl_version);
    $phpver = sprintf($LANG_MB00['php_check'],$php_version);

    $T->set_var(array(
        'lang_overview'     => $LANG_MB00['overview'],
        'mg_requirements'   => $LANG_MB00['preinstall_check'],
        'gl_version'        => $glver,
        'php_version'       => $phpver,
        'tc_installed'      => $tc_installed == 0 ? $LANG_MB01['no'] : $LANG_MB01['yes'],
        'install_doc'       => $LANG_MB00['preinstall_confirm'],
        'lang_template_cache'    => $LANG_MB00['template_cache'],
    ...
    if ( $errCheck == 0 ) {
        ...
        $T->set_var('btnmsg', $LANG_MB00['recheck_env']);
        $T->set_var('action','recheck');
        $T->set_var('errormessage',$LANG_MB00['fix_install']);
    }
} else {
   echo COM_refresh($_CONF['site_admin_url'] . '/plugins/menubuilder/index.php?mode=install');
   exit;
}
