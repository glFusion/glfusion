...
// +---------------------------------------------------------------------------+
// | $Id:: index.php 1990 2008-02-13 04:22:12Z mevans0263                     $|
// | Gallery 2 import wizard (written for Gallery 2.1)                         |
// +---------------------------------------------------------------------------+
...
function MG_buildImportAlbums( $galleryDir ) {
    ...
    $sql = "SELECT * FROM " . $storeConfig['tablePrefix'] . "Entity AS e LEFT JOIN " . $storeConfig['tablePrefix'] . "ChildEntity AS c ON e." . $storeConfig['columnPrefix'] . "id=c." . $storeConfig['columnPrefix'] . "id LEFT JOIN " . $storeConfig['tablePrefix'] . "Item AS i ON i." . $storeConfig['columnPrefix'] . "id=c." . $storeConfig['columnPrefix'] . "id WHERE " . $storeConfig['columnPrefix'] . "parentId=0 AND "  . $storeConfig['columnPrefix'] . "entityType='GalleryAlbumItem'";
    $tlResult = @mysql_query($sql,$dbImport);
    while ( $tlRow = @mysql_fetch_array($tlResult,MYSQL_BOTH)) {
        $currentPid = $tlRow[$storeConfig['columnPrefix'] . 'id'];

        $sql = "SELECT * FROM " . $storeConfig['tablePrefix'] . "Entity AS e LEFT JOIN " .
                $storeConfig['tablePrefix'] . "ChildEntity AS c ON e." . $storeConfig['columnPrefix'] .
                "id=c." . $storeConfig['columnPrefix'] . "id LEFT JOIN " . $storeConfig['tablePrefix'] .
                "Item AS i ON i." . $storeConfig['columnPrefix'] . "id=c." . $storeConfig['columnPrefix'] . "id " .
                " LEFT JOIN " . $storeConfig['tablePrefix'] . "FileSystemEntity AS f ON c." . $storeConfig['columnPrefix'] . "id=f." . $storeConfig['columnPrefix'] . "id " .
                "WHERE " . $storeConfig['columnPrefix'] . "parentId<>0 AND "  . $storeConfig['columnPrefix'] . "entityType='GalleryAlbumItem'";
        $result = @mysql_query($sql,$dbImport);

        while ( $row = @mysql_fetch_array($result, MYSQL_BOTH)) {
            $mgalbum = new mgAlbumg();
            $mgalbum->id            = $row[$storeConfig['columnPrefix'] . 'id'];
            $mgalbum->galleryName   = $row[$storeConfig['columnPrefix'] . 'title'];
            $mgalbum->title         = $row[$storeConfig['columnPrefix'] . 'title'];
            if ( $row[$storeConfig['columnPrefix'] . 'parentId'] == $currentPid ) {
                $mgalbum->parent = 0;
            } else {
                $mgalbum->parent        = $row[$storeConfig['columnPrefix'] . 'parentId'];
            }
            $mgalbum->description   = $row[$storeConfig['columnPrefix'] . 'description'];
            $mgalbum->media_count   = 0; // $photoCount;
            $mgalbum->galleryPath   = $row[$storeConfig['columnPrefix'] . 'pathComponent'] . '/';

            $ownerId = $row[$storeConfig['columnPrefix'] . 'ownerId'];
            $sql = "SELECT * FROM " . $storeConfig['tablePrefix'] . "User WHERE " . $storeConfig['columnPrefix'] . "id=" . $ownerId;
            $ownerResult = @mysql_query($sql, $dbImport);
            $ownerRow    = @mysql_fetch_array($ownerResult,MYSQL_BOTH);
            if ( $ownerRow[$storeConfig['columnPrefix'] . 'userName'] != '' ) {
                $mgalbum->galleryOwner  = $ownerRow[$storeConfig['columnPrefix'] . 'userName'];
            } else {
                $mgalbum->galleryOwner = 'admin';
            }

            $mgalbum->counter = $counter;
            $mgAlbums[$row[$storeConfig['columnPrefix'] . 'id']] = $mgalbum;
            $counter++;
        }
    }
...
function MG_importSelectAlbums($galleryDir ) {
    ...
    $T->set_var('form_action',$_MG_CONF['admin_url'] . '/importers/gallery2/index.php');
    $T->set_var(array(
...
function MG_importAlbums( $aid, $parent, $galleryDir, $session_id=0 ) {
    ...
    for ($i=0; $i < $nrows; $i++ ) {
        ...
        if ( $_POST['gallery'][$x] == 1 ) {
            ...
            COM_errorLog("Media Gallery: Gallery v2 Import processed " . $mgAlbums[$children[$i]]->title . " MGID: " . $mgAlbums[$children[$i]]->mgid . " Parent: " . $mgAlbums[$children[$i]]->mgparent);

            $galleryDir  = $storeConfig['galleryBase'] . 'albums/' . $mgAlbums[$children[$i]]->getPath();
...
function MG_importFiles( $album_id, $import_album_id, $galleryDir, $session_id ) {
    ...
    $sql = "SELECT * FROM " . $storeConfig['tablePrefix'] .
           "Entity AS e LEFT JOIN " . $storeConfig['tablePrefix'] . "ChildEntity AS c ON e." .
           $storeConfig['columnPrefix'] . "id=c." . $storeConfig['columnPrefix'] .
           "id LEFT JOIN " . $storeConfig['tablePrefix'] . "Item AS i ON i." .
           $storeConfig['columnPrefix'] . "id=c." . $storeConfig['columnPrefix'] . "id " .
           " LEFT JOIN " . $storeConfig['tablePrefix'] . "FileSystemEntity AS f ON i." .
           $storeConfig['columnPrefix'] . "id=f." . $storeConfig['columnPrefix'] . "id" .
           " WHERE " . $storeConfig['columnPrefix'] . "parentId=" . $import_album_id;

    $tlResult = @mysql_query($sql,$dbImport);
    while ( $tlRow = @mysql_fetch_array($tlResult,MYSQL_BOTH)) {
        if ( $tlRow[$storeConfig['columnPrefix'] . 'entityType'] == 'GalleryPhotoItem' || $tlRow[$storeConfig['columnPrefix'] . 'entityType'] == 'GalleryMovieItem' || $tlRow[$storeConfig['columnPrefix'] . 'entityType'] == 'GalleryUnknownItem' ) {
            $fullFileName = $galleryDir . $tlRow[$storeConfig['columnPrefix'] . 'pathComponent'];
            $fileName     = $tlRow[$storeConfig['columnPrefix'] . 'pathComponent'];

            $fileList[$counter]['aid'] = $album_id;
            $fileList[$counter]['data'] = $fullFileName;
            $fileList[$counter]['data2'] = $tlRow[$storeConfig['columnPrefix'] . 'pathComponent']; // $tlRow['g_title'];
            $fileList[$counter]['data3'] = $tlRow[$storeConfig['columnPrefix'] . 'description'];
            $counter++;
        }
    }
    @mysql_close($dbImport);
...
if (isset($_POST['mode']) ) {
    ...
    $storeConfig['hostname'] = COM_applyFilter($_POST['hostname']);
    ...
    $storeConfig['columnPrefix'] = COM_applyFilter($_POST['column_prefix']);
    ...
    // check to see if we have the ending slash on directory...
    ...
    switch ($mode) {
        case 'next' :
            ...
            if (!@is_dir($storeConfig['galleryBase'] )) {
                $display .= MG_errorHandler( 'This does not appear to be the proper path to Gallery base directory' );
                $display .= MG_siteFooter();
                echo $display;
                exit;
            }
            $albumdir = $storeConfig['galleryBase'] . 'albums/';
            if (!@is_dir($albumdir )) {
                $display .= MG_errorHandler( 'Unable to find albums/ directory under galleryBase' );
                $display .= MG_siteFooter();
                echo $display;
                exit;
            }
            ...
            break;
        case 'convert' :
            $session_description = 'Gallery v2 Import';
            $session_id = MG_beginSession('gallery2import',$_MG_CONF['site_url'] . '/index.php',$session_description );
...
$T->set_file (array('page' => 'import_gallery2_parms.thtml'));
$T->set_var('form_action',$_CONF['site_admin_url'] . '/plugins/mediagallery/importers/gallery2/index.php');
$T->set_var('import_wizard','Gallery2');

$T->set_var(array(
    'lang_gallery_database'     => 'Gallery v2 Database Parameters',
    'lang_hostname'             => 'Hostname',
    'lang_database'             => 'Database',
    'lang_username'             => 'Username',
    'lang_password'             => 'Password',
    'lang_table_prefix'         => 'Table Prefix',
    'lang_column_prefix'        => 'Column Prefix',
    'lang_gallery_directory'    => 'Gallery v2 Base Directory',
));
